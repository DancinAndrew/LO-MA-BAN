...
class ReportGeneratorService:
    ...
    def _render_llm_quiz(self, quiz_data: dict[str, Any]) -> dict[str, Any]:
        ...
        for i, opt in enumerate(options[:4]):
            if isinstance(opt, dict):
                oid, otxt = opt.get("id", ids[i] if i < 4 else str(i + 1)), opt.get("text", str(opt))
            else:
                oid, otxt = (ids[i] if i < 4 else str(i + 1)), str(opt)
            ...
            fmt.append({
                "id": str(oid), "text": self._simplify_text(str(otxt), 100),
    ...
    def _content_risk_quiz(self) -> dict[str, Any]:
        return {
            "enabled": True,
        ...
        for i, rec in enumerate(recommendations[:3]):
            ...
            if clean:
                ...
                tips.append({"id": f"tip_{i + 1}", "icon": t["icon"],
                             "tip": self._simplify_text(clean, 60), "why": t["why"], "action_text": "記住囉！"})
        while len(tips) < 4:
